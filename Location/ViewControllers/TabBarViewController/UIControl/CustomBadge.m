...
-(void) drawRoundedRectWithContext:(CGContextRef)context withRect:(CGRect)rect
{
	...
	CGFloat puffer = CGRectGetMaxY(rect)*0.1;
...
- (void)drawRect:(CGRect)rect {
	...
	if ([self.badgeText length]>0) {
        UIFont * fonts = [UIFont boldSystemFontOfSize:8];
        
		CGFloat sizeOfFont = fonts.pointSize*badgeScaleFactor;// 13.5*badgeScaleFactor;
		if ([self.badgeText length]<2) {
			//sizeOfFont += sizeOfFont*0.20;
		}
		UIFont *textFont = [UIFont boldSystemFontOfSize:sizeOfFont];
		CGSize textSize = [badgeText sizeWithAttributes:@{NSFontAttributeName:textFont}];
