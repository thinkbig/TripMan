...
@property (nonatomic, strong) UIImage *hostBgImage;
...
- (void)refreshEditingButtons
{
    ...
    [self.rzEditingItems enumerateObjectsUsingBlock:^(RZCollectionTableViewCellEditingItem *item, NSUInteger idx, BOOL *stop) {
        ...
        button.backgroundColor = item.bgColor ? item.bgColor : [UIColor clearColor];
        ...
        if ( idx == self.editingButtons.count ) {
            if (item.hostBgImage) {
                UIImageView * oldBg = (UIImageView*)[self.editingButtonsHostView viewWithTag:20001];
                if (nil == oldBg || ![oldBg isKindOfClass:[UIImageView class]]) {
                    oldBg = [[UIImageView alloc] initWithFrame:self.editingButtonsHostView.bounds];
                    oldBg.tag = 20001;
                    oldBg.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
                    [self.editingButtonsHostView addSubview:oldBg];
                    [self.editingButtonsHostView sendSubviewToBack:oldBg];
                }
                oldBg.image = item.hostBgImage;
            } else {
