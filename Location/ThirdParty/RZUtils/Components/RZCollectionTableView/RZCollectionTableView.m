...
- (void)_rz_editingButtonPressed:(NSUInteger)buttonIdx forCell:(RZCollectionTableViewCell *)cell
{
    if ( cell != nil && [self.visibleCells containsObject:cell] ) {
        ...
        } else if ( [self.delegate respondsToSelector:@selector(collectionView:rzTableLayout:editingButtonPressedForIndex:forRowAtIndexPath:)] ) {
            [(id<RZCollectionTableViewLayoutDelegate>)(self.delegate) collectionView:self
                                  rzTableLayout:(RZCollectionTableViewLayout*)self.collectionViewLayout
