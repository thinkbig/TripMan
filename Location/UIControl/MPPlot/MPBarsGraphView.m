...
- (void)addBarsAnimated:(BOOL)animated
{
    ...
    for (NSInteger i=0; i<points.count; i++) {
        ...
        button.frame=CGRectMake(xPos, animated ? self.height : self.height-height, barWidth, animated ? height+20 : height);
        ...
        if (animated) {
            [UIView animateWithDuration:self.animationDuration delay:i*0.1 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
                ...
                if (self.coorDelegate) {
                    [self.coorDelegate didLayoutIndex:i ofTopPoint:CGPointMake(xPos+barWidth/2.0, height)];
                }
                [UIView animateWithDuration:.15 animations:^{
                    button.frame = CGRectMake(xPos, self.height-height, barWidth, height);
                }];
            }];
        }
